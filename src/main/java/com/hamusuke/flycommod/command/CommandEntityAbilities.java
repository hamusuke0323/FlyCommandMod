...
public class CommandEntityAbilities {
	public static void register(CommandDispatcher<CommandSource> dispatcher) {
		LiteralArgumentBuilder<CommandSource> literalArgumentBuilder = Commands.literal("entityabilities").requires((permission) -> permission.hasPermissionLevel(2));
		...
		literalArgumentBuilder.then(Commands.argument("targets", EntityArgument.entities()).then(Commands.literal("noGravity").then(Commands.argument("boolean", BoolArgumentType.bool()).executes((command) -> {
			...
			return noGravity(command.getSource(), EntityArgument.getEntities(command, "targets").stream().filter(entity -> entity.hasNoGravity() != flag).collect(Collectors.toList()), flag);
		}))));
	...
	private static int noGravity(CommandSource source, Collection<? extends Entity> entities, boolean flag) {
		entities.forEach((entity) -> {
			...
			if (!flag) {
				entity.fallDistance = -(float) (entity.getPosY() + 10.0D);
			}
		});

		if (entities.size() == 1) {
			source.sendFeedback(new TranslationTextComponent("hamusuke.command.entityabilities.success.nogravity." + flag + ".single", entities.iterator().next().getDisplayName()), true);
		} else {
			source.sendFeedback(new TranslationTextComponent("hamusuke.command.entityabilities.success.nogravity." + flag + ".multiple", entities.size()), true);
		}
	...
	private static int setGlowing(CommandSource source, Collection<? extends Entity> entities, boolean flag) {
		...
		if (entities.size() == 1) {
			source.sendFeedback(new TranslationTextComponent("hamusuke.command.entityabilities.success.setglowing." + flag + ".single", entities.iterator().next().getDisplayName()), true);
		} else {
			source.sendFeedback(new TranslationTextComponent("hamusuke.command.entityabilities.success.setglowing." + flag + ".multiple", entities.size()), true);
		}
	...
	private static int setInvulnerable(CommandSource source, Collection<? extends Entity> entities, boolean flag) {
		...
		if (entities.size() == 1) {
			source.sendFeedback(new TranslationTextComponent("hamusuke.command.entityabilities.success.setinvulnerable." + flag + ".single", entities.iterator().next().getDisplayName()), true);
		} else {
			source.sendFeedback(new TranslationTextComponent("hamusuke.command.entityabilities.success.setinvulnerable." + flag + ".multiple", entities.size()), true);
		}
	...
	private static int setInvisible(CommandSource source, Collection<? extends Entity> entities, boolean flag) {
		...
		if (entities.size() == 1) {
			source.sendFeedback(new TranslationTextComponent("hamusuke.command.entityabilities.success.invisible." + flag + ".single", entities.iterator().next().getDisplayName()), true);
		} else {
			source.sendFeedback(new TranslationTextComponent("hamusuke.command.entityabilities.success.invisible." + flag + ".multiple", entities.size()), true);
		}
