...
public class FlyCommand {
	public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
		...
		}).then(Commands.argument("targets", EntityArgument.players()).then(Commands.literal("allow").executes((e) -> {
	...
	private static int allow(CommandContext<CommandSourceStack> source, Collection<ServerPlayer> players) {
		...
		if (players.size() == 1) {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.allow.single", players.iterator().next().getDisplayName()), true);
		} else {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.allow.multiple", players.size()), true);
		}
	...
	private static int disallow(CommandContext<CommandSourceStack> source, Collection<ServerPlayer> players) {
		players.forEach((serverPlayer) -> {
			...
			serverPlayer.onUpdateAbilities();
			serverPlayer.fallDistance = -(float) (serverPlayer.getY() + 10.0D);
		});

		if (players.size() == 1) {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.noallow.single", players.iterator().next().getDisplayName()), true);
		} else {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.noallow.multiple", players.size()), true);
		}
	...
	private static int noGravity(CommandContext<CommandSourceStack> source, Collection<ServerPlayer> players) {
		...
		if (players.size() == 1) {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.nogravity.single", players.iterator().next().getDisplayName()), true);
		} else {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.nogravity.multiple", players.size()), true);
		}
	...
	private static int gravity(CommandContext<CommandSourceStack> source, Collection<ServerPlayer> players) {
		...
		if (players.size() == 1) {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.gravity.single", players.iterator().next().getDisplayName()), true);
		} else {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.gravity.multiple", players.size()), true);
		}
	...
	private static int getFlySpeed(CommandContext<CommandSourceStack> source, Collection<ServerPlayer> players) {
		players.forEach((serverPlayer) -> source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.getflyspeed", serverPlayer.getDisplayName(), serverPlayer.getAbilities().getFlyingSpeed()), false));
		return players.size();
	}

	private static int getWalkSpeed(CommandContext<CommandSourceStack> source, Collection<ServerPlayer> players) {
		players.forEach((serverPlayer) -> source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.getwalkspeed", serverPlayer.getDisplayName(), serverPlayer.getAbilities().getWalkingSpeed()), false));
		return players.size();
	}
	...
	private static int setFlySpeed(CommandContext<CommandSourceStack> source, Collection<ServerPlayer> players, float flySpeed) {
		...
		if (players.size() == 1) {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.setflyspeed.single", players.iterator().next().getDisplayName(), flySpeed), true);
		} else {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.setflyspeed.multiple", players.size(), flySpeed), true);
		}
	...
	private static int setWalkSpeed(CommandContext<CommandSourceStack> source, Collection<ServerPlayer> players, float walkSpeed) {
		...
		if (players.size() == 1) {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.setwalkspeed.single", players.iterator().next().getDisplayName(), walkSpeed), true);
		} else {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.setwalkspeed.multiple", players.size(), walkSpeed), true);
		}
	...
	private static int allowEdit(CommandContext<CommandSourceStack> source, Collection<ServerPlayer> players, boolean flag) {
		...
		if (players.size() == 1) {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.allowedit." + flag + ".single", players.iterator().next().getDisplayName()), true);
		} else {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.allowedit." + flag + ".multiple", players.size()), true);
		}
	...
	private static int disableDamage(CommandContext<CommandSourceStack> source, Collection<ServerPlayer> players, boolean flag) {
		...
		if (players.size() == 1) {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.disabledamage." + flag + ".single", players.iterator().next().getDisplayName()), true);
		} else {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.disabledamage." + flag + ".multiple", players.size()), true);
		}
	...
	private static int isFlying(CommandContext<CommandSourceStack> source, Collection<ServerPlayer> players, boolean flag) {
		...
		if (players.size() == 1) {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.isflying." + flag + ".single", players.iterator().next().getDisplayName()), true);
		} else {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.isflying." + flag + ".multiple", players.size()), true);
		}
	...
	private static int isGlowing(CommandContext<CommandSourceStack> source, Collection<ServerPlayer> players, boolean flag) {
		...
		if (players.size() == 1) {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.setglowing." + flag + ".single", players.iterator().next().getDisplayName()), true);
		} else {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.setglowing." + flag + ".multiple", players.size()), true);
		}
	...
	private static int isInvulnerable(CommandContext<CommandSourceStack> source, Collection<ServerPlayer> players, boolean flag) {
		...
		if (players.size() == 1) {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.setinvulnerable." + flag + ".single", players.iterator().next().getDisplayName()), true);
		} else {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.setinvulnerable." + flag + ".multiple", players.size()), true);
		}
	...
	private static int isInvisible(CommandContext<CommandSourceStack> source, Collection<ServerPlayer> players, boolean flag) {
		...
		if (players.size() == 1) {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.invisible." + flag + ".single", players.iterator().next().getDisplayName()), true);
		} else {
			source.getSource().sendSuccess(new TranslatableComponent("hamusuke.command.fly.success.invisible." + flag + ".multiple", players.size()), true);
		}
